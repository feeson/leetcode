...
class Solutionp17 {
    ...
    boolean dfs(TreeNode root,int val,int targetSum){
        if (root.left==null&&root.right==null)
            return val+root.val==targetSum;
        if (root.left==null)
            return dfs(root.right,val+root.val,targetSum);
        if (root.right==null)
            return dfs(root.left,val+root.val,targetSum);
        return dfs(root.left,val+root.val,targetSum)||dfs(root.right,val+root.val,targetSum);
    }
}